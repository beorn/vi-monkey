...
describe('gen()', () => {
	...
	it('generates from weighted picker', async () => {
		...
		// 80% a, 15% b, 5% c
		for await (const v of take(gen([[80, 'a'], [15, 'b'], [5, 'c']], 42), 1000)) {
	...
	it('flattens array results from picker', async () => {
		...
		for await (const v of take(gen(() => ['x', 'y', 'z'], 42), 6)) {
			values.push(v)
		}
...
describe('shrinkSequence()', () => {
	...
	it('respects minLength option', async () => {
		...
		const result = await shrinkSequence(['a', 'b', 'c', 'd', 'e'], runTest, { minLength: 3 })
